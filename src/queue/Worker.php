...
use iboxs\Queue;
use iboxs\queue\event\JobExceptionOccurred;
use iboxs\queue\event\JobFailed;
use iboxs\queue\event\JobProcessed;
use iboxs\queue\event\JobProcessing;
use iboxs\queue\event\WorkerStopping;
use iboxs\queue\exception\MaxAttemptsExceededException;
use Throwable;
...
class Worker
{
     ...
     * @param int    $timeout
     */
    public function amqpDaemon($connection, $queue, $delay = 0, $sleep = 3, $maxTries = 0, $memory = 128, $timeout = 60)
    {
        ...
        while (true) {
            ...
            $this->queue->connection($connection)->consume($this, $lastRestart, $connection, $queue, $delay, $sleep, $maxTries);

        }
    }

    //amqp操作
    public function amqpAction($job, $lastRestart, $connection, $queue, $delay = 0, $sleep = 3, $maxTries = 0, $memory = 128, $timeout = 60)
    {
    ...
    {
        if ($this->cache) {
            return $this->cache->get('iboxs:queue:restart');
        }
    }
